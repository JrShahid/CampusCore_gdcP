...
public final class AppAuthCoordinator {
        ...
        auth.createUserWithEmailAndPassword(email, password)
                .addOnCompleteListener(activity, task -> {
                    if (!task.isSuccessful() || auth.getCurrentUser() == null) {
                        AuthOutcome outcome = mapFirebaseFailure(task.getException());
                        if (outcome.getStatus() == AuthOutcome.Status.REMOTE_ERROR) {
                            callback.onResult(registerLocalStudent(activity, name, email, password, className));
                            return;
                        }
                    ...
                    firestore.collection("users")
                            ...
                            .addOnSuccessListener(unused -> {
                                ...
                                UserAccountRepository.registerAccount(
                                        activity,
                                        name,
                                        email,
                                        password,
    ...
    public static void login(
        ...
        auth.signInWithEmailAndPassword(email, password)
                .addOnCompleteListener(activity, task -> {
                    if (!task.isSuccessful() || auth.getCurrentUser() == null) {
                        AuthOutcome outcome = mapFirebaseLoginFailure(task.getException());
                        if (outcome.getStatus() == AuthOutcome.Status.REMOTE_ERROR) {
                            UserAccountRepository.AuthResult localResult =
                                    UserAccountRepository.authenticate(activity, email, password);
                            callback.onResult(mapLocalAuthResult(localResult));
                            return;
                        }
                        callback.onResult(outcome);
                        return;
    ...
    private static AuthOutcome registerLocalStudent(
            Activity activity,
            String name,
            String email,
            String password,
